 ...
 */
/**ltl
功能:中断控制器描述符，在这个结构里的函数指针指向与中断控制器电路(PIC)打交道的IO例程。
注:在linux kernel中支持的中断控制器有:8259A、SMP IO-APIC、PIIX4的内部8259 PIC、及SGI的visual workstation (IO-)APIC
*/
struct irq_chip {
	const char	*name;
...
struct irq_desc {
	void fastcall		(*handle_irq)(unsigned int irq,
					      ...
					      struct pt_regs *regs);
	struct irq_chip		*chip; /* 中断控制器芯片的操作函数集 */
	void			*handler_data;
	void			*chip_data;
	/* 单链表头指针，这个链表由共享同一中断号的中断服务例程描述的链接而成 */
	struct irqaction	*action;	/* IRQ action list */
	/* 中断的状态 */
	unsigned int		status;		/* IRQ status */
	/*如果启动这个中断，则depth=0,如果禁用了这个中断多次，则这个值为正数，即每调用一次disable_irq(),这个值就+1*/
...
extern struct irq_desc irq_desc[NR_IRQS];/*ltl:NR_IRQS=224*/
