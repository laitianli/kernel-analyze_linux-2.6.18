...
/**ltl
功能:实现PCI外设IO物理内存空间到虚拟地址的映射
参数:dev	->pci设备对象
	bar	->pci设备配置的BAR空间下标(0~5)
	maxlen	->bar空间的最大值
返回值:虚拟地址
说明:linux kernel的中断、内存空间、IO空间统称为资源
	若bar空间是IO空间，实现IO空间到虚拟地址映射
	若bar空间是内存空间，实现内存空间到虚拟地址映射。
*/
/* Create a virtual mapping cookie for a PCI BAR (memory or IO) */
void __iomem *pci_iomap(struct pci_dev *dev, int bar, unsigned long maxlen)
{
	unsigned long start = pci_resource_start(dev, bar);//起始地址
	unsigned long len = pci_resource_len(dev, bar);		//空间大小
	unsigned long flags = pci_resource_flags(dev, bar);	//资源类型

	if (!len || !start)
		return NULL;
	if (maxlen && len > maxlen)
		len = maxlen;
	if (flags & IORESOURCE_IO)//实现IO空间映射
		return ioport_map(start, len);
	if (flags & IORESOURCE_MEM) {//实现内存空间映射
		if (flags & IORESOURCE_CACHEABLE)//内存空间支持预读取
			return ioremap(start, len);
		return ioremap_nocache(start, len);//不能预读取
	}
	/* What? */
	return NULL;
}
