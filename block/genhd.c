...
/**ltl
功能:为一个块设备分配主设备号
参数:major	->主设备号，如果此值为0,表明自动分配一个。
	name	->设备号名称
返回值:如果参数major不为0，而返回值为0，表示分配成功；如果major为0，返回值大于0,表明分配到的主设备号；如果返回值小于0，则分配失败。
*/
...
struct gendisk *alloc_disk_node(int minors, int node_id)
{
	...
	if (disk) {
		...
		/*如果存在子分区时，则分配区表。注:当minors==1时，就不存在子分区*/
		if (minors > 1) {
