...
#define rq_hash_key(rq)		((rq)->sector + (rq)->nr_sectors) //Hash的Key值为请求的最后一扇区号
...
struct deadline_data {
	...
	//为了更加容易判定bio能否在request之后合并<deadline_merge>
...
struct deadline_rq {
	...
	sector_t rb_key;		//当发生了两个request合并的话，这个值记录的是合并之前的rb key
...
{
	struct hlist_head *hash_list = &dd->hash[DL_HASH_FN(offset)];//从hash表中获取offset的request所在的列表头
...
static struct elevator_type iosched_deadline = {
	.ops = {
		.elevator_merge_fn = 		deadline_merge,//查找可以和bio进行合并的request.
		.elevator_merged_fn =		deadline_merged_request,//有请求被合并时被调用，即有bio合并到一个request后被调用
		.elevator_merge_req_fn =	deadline_merged_requests,//合并两个request
		.elevator_dispatch_fn =		deadline_dispatch_requests,//将准备好的请求request派发到派发队列中。
		.elevator_add_req_fn =		deadline_add_request,//往调度器中添加一个新请求时被调用
		.elevator_queue_empty_fn =	deadline_queue_empty,//判定电梯算法队列是否为空
		.elevator_former_req_fn =	deadline_former_request,//返回按照磁盘排序顺序在给定请求前面的那个请求，被块层用来查找合并可能性。
		.elevator_latter_req_fn =	deadline_latter_request,//返回按照磁盘排序顺序在给定请求后面的那个请求，被块层用来查找合并可能性。
		.elevator_set_req_fn =		deadline_set_request,//用于为请求分配私有存储空间
		.elevator_put_req_fn = 		deadline_put_request,//释放私有存储空间
		.elevator_init_fn =		deadline_init_queue,	//初始化调度器的私有数据
		.elevator_exit_fn =		deadline_exit_queue,	//释放调度器的私有数据
	},

	.elevator_attrs = deadline_attrs,//调度器的私有属性(在blk_register_queue->elv_register_queue中创建)
	.elevator_name = "deadline",		//调度器的名字
