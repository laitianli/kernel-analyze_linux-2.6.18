...
inline int elv_rq_merge_ok(struct request *rq, struct bio *bio)
{
	 ...
	 */
	 //3.1.对同一个磁盘的请求，3.2.request是来自应用进程(在init_request_from_bio中设置rq->waiting=NULL)
...
static inline int elv_try_merge(struct request *__rq, struct bio *bio)
{
	...
	if (elv_rq_merge_ok(__rq, bio)) {//bio符合合并的条件下
		if (__rq->sector + __rq->nr_sectors == bio->bi_sector)//request的最后一个扇区刚好是bio的起始扇区,则把bio合到request中的bio链表尾
			ret = ELEVATOR_BACK_MERGE;
		else if (__rq->sector - bio_sectors(bio) == bio->bi_sector)//request的起始扇区刚好是bio的最后扇区，则把bio合到request的bio链表头
...
void elv_insert(request_queue_t *q, struct request *rq, int where)
{
	...
	switch (where) {
	case ELEVATOR_INSERT_FRONT://scsi命令插入
		rq->flags |= REQ_SOFTBARRIER;

		list_add(&rq->queuelist, &q->queue_head);  //在队列头部插入
		break;
...
static inline struct request *__elv_next_request(request_queue_t *q)
{
	...
	while (1) {
		...
		//如果派发队列为空的话，就要从IO调度队列转移请求到派发队列中。
