...
static int usb_stor_msg_common(struct us_data *us, int timeout)
{
	...
	/*current_urb->setup_dma和transfer_dma已在associate_dev函数中已经分配
		associate_dev已经建立起了两个DMA映射:us->cr <-> us->cr_dma
								   us->iobuf	<-> us->iobuf_dma
		因此要设置URB_NO_SETUP_DMA_MAP标志，以防在hcd_submit_urb重新分配这两个buf								   
	*/
...
参数:	us		->u盘对象
	pipe		->管道
	sg		->scatter-gather list object
	num_sg	->scatter-gather list element num
	length	->scatter-gather list data length
	act_len	->[out]传输成功的实际长度。
返回值:
说明:要传递给u盘的数据以scatter-gather list形式存在
*/
		...
		unsigned int *act_len)
{
	...
	US_DEBUGP("%s: xfer %u bytes, %d entries\n", __FUNCTION__,
			length, num_sg);
	//对scatter-gather list的各个元素建立DMA映射，同时创建urb对象
...
int usb_stor_Bulk_transport(struct scsi_cmnd *srb, struct us_data *us)
{
	...
	bcb->DataTransferLength = cpu_to_le32(transfer_length);//传输数据的长度
	bcb->Flags = srb->sc_data_direction == DMA_FROM_DEVICE ? 1 << 7 : 0;//方向
	bcb->Tag = ++us->tag;
	bcb->Lun = srb->device->lun;//设置逻辑号
	if (us->flags & US_FL_SCM_MULT_TARG)
		bcb->Lun |= srb->device->id << 4;
	bcb->Length = srb->cmd_len;//scsi命令长度

	/* copy the command payload */
	memset(bcb->CDB, 0, sizeof(bcb->CDB));
	memcpy(bcb->CDB, srb->cmnd, bcb->Length);//设置scsi命令
