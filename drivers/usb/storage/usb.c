...
功能:控制线程
参数:us->	 u盘数据结构
说明:用来传输数据，在线程开始时，down_interruptible会使用线程sleep,在scsiglue.c:queuecommand会调用up唤醒进程。
*/
 ...
 ***********************************************************************/
/**ltl
功能:u盘对象与usb设备接口对象相关联。
	1.设置u盘对象的数据成员。
	2.设置usb设备对私有数据成员
	3.分配setup阶段数据包对象、数据阶段的数据包、感测数据
*/
...
static int get_device_info(struct us_data *us, const struct usb_device_id *id)
{
	...
	//subclass:6,protocol:80,flags:0
...
/**ltl
功能:设置transport接口，这接口用来传输数据到usb子系统的直接接口
参数:us	->u盘数据结构
返回:
说明:protocol字段值为接口bInterfaceSubClass字段值
*/
...
/**ltl
功能:根据interface protocol设置处理函数。
参数
返回值:
说明:处理函数是由transport实现。
*/
...
static int usb_stor_acquire_resources(struct us_data *us)
{
	...
	wake_up_process(th);//调用这个函数后，kthread_create创建的线程才能运行
			 ...
			 const struct usb_device_id *id)
{
	...
	if (result)
		goto BadDevice;
	//数据传输线程和数据传输urb对象current_urb在此创建。
