...
static int intr_submit (
	...
	/*分配ehci_qh对象，并保存到eq->hcpriv,这里列表empty参数是NULL，主要作用是分配ehci_qh对象*/
...
)
{
	...
	if (dev->speed == USB_SPEED_HIGH) {
		unsigned multi = hb_mult(maxp);//此次传输由几个事务完成
...
)
{
	...
	for (i = 0; i < urb->number_of_packets; i++) {
		...
		if (unlikely (((i + 1) == urb->number_of_packets))
			...
			trans |= EHCI_ITD_IOC;//表示事务传输完成后，产生中断
		trans |= length << 16;
...
)
{
	...
	int			status;
	unsigned		mod = ehci->periodic_size << 3;//Q:why?periodic_size表示段数，乘8后才表示数据长度
	...
	now = readl (&ehci->regs->frame_index) % mod;//frame list index
	...
	gfp_t mem_flags)
{
	...
	/* Get iso_stream head */
	/*获取与endpoint相应的ehci_iso_stream对象，如果为NULL
		则分配一个新的对象，并对其成员赋值。
	*/
