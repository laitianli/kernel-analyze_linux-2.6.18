...
参数:
返回值:
说明:对一控制请求，执行必须是同步的，因此，要用complete来保证
*/
...
{ 
	...
	urb->context = &done;
	/*actual_length执行urb后，返回的实际长度，
	  ehci_q.c:qtd_copy_status中赋值
	*/
	urb->actual_length = 0;
	//提交urb请求。
...
参数:usb_dev	->usb设备对象
	pipe		->管道
	cmd		->setup事务的8字节的数据(struct usb_ctrlrequest)。
	data		->数据
	len		->长度
	timout	->超时
返回值:
...
参数:dev		->
	pipe		->
	request	->请求
	requesttype	->请求类型,见usb 2.0 spec table 9-4
	value		->
	index		->
	data		->
	size		->
 ...
 */
/**ltl
功能:当usb请求的数据以scatterlist形式存在时，此函数对这些请求进行初始化，包括DMA映射
参数:
返回值:
	...
	for (i = 0; i < io->entries; i++) {
		...
		if (dma) {
			...
			io->urbs [i]->transfer_buffer =
				page_address (sg [i].page) + sg [i].offset;//返回page对应的虚拟地址
			len = sg [i].length;
		}
