...
/**ltl
 * 功能: 设置PCI设备的MSI功能中的某个中断开启与关闭。
 * 参数:
 * 返回值:
 * 说明: 注:对于MSI中断方式，把相应的屏蔽位置0,即开启相应的中断号
 */
...
static int assign_msi_vector(void)
{
	...
	if (!new_vector_avail) {
		...
		nr_released_vectors--;
		spin_unlock_irqrestore(&msi_lock, flags);
		if (msi_desc[free_vector] != NULL) { /* 释放MSI描述符 */
			struct pci_dev *dev;
			int tail;
...
static void irq_handler_init(int cap_id, int pos, int mask)
{
	...
	if (cap_id == PCI_CAP_ID_MSIX)
		irq_desc[pos].chip = &msix_irq_type;	/* MSIX中断方式下的中断控制器操作函数集 */
	else {
		if (!mask)
			irq_desc[pos].chip = &msi_irq_wo_maskbit_type; /* 不带屏蔽位 */
		else		
			irq_desc[pos].chip = &msi_irq_w_maskbit_type;	/* 带屏蔽位 */
	}
...
static int msi_register_init(struct pci_dev *dev, struct msi_desc *entry)
{
	...
	if (is_64bit_address(control)) { /* 为64位情况下，写放高地址 */
		pci_write_config_dword(dev,
			msi_upper_address_reg(pos), address_hi);
		pci_write_config_word(dev,
			msi_data_reg(pos, 1), data); /* 数据写入到高位 */
	} else
		pci_write_config_word(dev,
			msi_data_reg(pos, 0), data); /* 将data写入到低位 */
	if (entry->msi_attrib.maskbit) { /* 存在屏蔽位的情况下 */
...
static int msi_capability_init(struct pci_dev *dev)
{
	...
	entry = alloc_msi_entry(); /* 分配MSI描述符(从高速缓冲区中分配) */
	if (!entry)
		return -ENOMEM;
	/* 分配中断向量 */
	...
	dev->irq = vector; /* 把新分配的中断向量赋给设备irq */
	entry->dev = dev;	/* PCI设备对象 */
	if (is_mask_bit_support(control)) {
		entry->mask_base = (void __iomem *)(long)msi_mask_bits_reg(pos,
				is_64bit_address(control));
	}
	/* Replace with MSI handler */ 
	/* 设置新分配的中断向量的中断描述符的中断控制器芯片的操作函数集 */
...
int pci_enable_msi(struct pci_dev* dev)
{
	...
	if (!pos)
		return -EINVAL;
	/* 在msi描述符数组中查找dev的MSI描述符对象 */
	if (!msi_lookup_vector(dev, PCI_CAP_ID_MSI)) { 
		/* Lookup Sucess */
		unsigned long flags;
		/* MSI的控制信息 */
		pci_read_config_word(dev, msi_control_reg(pos), &control);
		if (control & PCI_MSI_FLAGS_ENABLE) /* 已经使能过 */
			return 0;	/* Already in MSI mode */
		spin_lock_irqsave(&msi_lock, flags);
		if (!vector_irq[dev->irq]) { /* <原因?> */
	...
	pos = pci_find_capability(dev, PCI_CAP_ID_MSIX);
	if (pos > 0 && !msi_lookup_vector(dev, PCI_CAP_ID_MSIX)) { /* 即支持MSI又支持MSIX的设备不存在 */
