...
static int sm_ioctl(struct block_device *bdev, fmode_t mode,unsigned int cmd, unsigned long arg);
...
static int sm_ioctl(struct block_device *bdev, fmode_t mode,
		    unsigned int cmd, unsigned long arg)
{
	struct gendisk *disk = bdev->bd_disk;
	struct scsi_device *sdp = (struct scsi_device*)(disk->private_data);
	void __user *p = (void __user *)arg;
	int error;
    
	Log("sd_ioctl: disk=%s, cmd=0x%x\n",disk->disk_name, cmd);
...
static int sm_revalidate_disk(struct gendisk *disk)
{
	Log("enter function.");
	return 0;
}
static struct block_device_operations sm_fops = {
	...
	.release = sm_release,
	.ioctl = sm_ioctl,
	.revalidate_disk = sm_revalidate_disk,
};
...
static int sm_probe(struct device* dev)
{
	...
	gd->major = SM_MAJOR + index;
	//gd->minors = 16;
	gd->first_minor = index;
	gd->fops = &sm_fops;
	gd->private_data = sdp;
	sprintf(gd->disk_name,"sm-scsi%d",index++);
...
static int __init sm_demo_init(void)
{
    int i = 0;
 //   int majors = 0;
  	for(i = 0; i < SM_MAJOR_COUNT; i++)
  	{
   		if(register_blkdev(SM_MAJOR + i,"sm")!=0)
...
static void __exit sm_demo_exit(void)
{
	int i = 0; 
	for(i = 0; i < SM_MAJOR_COUNT; i++)
		unregister_blkdev(SM_MAJOR + i,"sm");
