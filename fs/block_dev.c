...
struct block_device *bdget(dev_t dev)
{
	...
	if (!inode)/*获取设备的inode失败，返回NULL*/
		return NULL;

	/*通过设备文件的inode获取块设备描述符。*/
	bdev = &BDEV_I(inode)->bdev;

	/*如果inode对象是通过iget5_locked首次分配，则先对inode和bdev的各个成员初始化*/
	if (inode->i_state & I_NEW) {
		bdev->bd_contains = NULL;/*指定所属分区为NULL*/
		bdev->bd_inode = inode;
...
/**ltl
功能：打开块设备文件的接口，会调用到gendisk中的操作接口。同时，如果当前分区
为主分区时，去扫描主分区下的子分区。blkdev_get调用这个接口的主要目的就
去扫描子分区。
参数：bdev:块设备描述符。
      file:文件句柄指针。
      subclass:类型
...
do_open(struct block_device *bdev, struct file *file, unsigned int subclass)
{
	...
	mutex_lock_nested(&bdev->bd_mutex, subclass);
	/*如果bdev描述的块设备是首次打开，就要考虑两种情况：第一种打开的是主分
	  区（part==0），此时要调用底层驱动的打开接口，并扫描主分区下的子分区；第
	  二种是打开的就是一个子分区，*/
	if (!bdev->bd_openers) {
		...
		*/
		...
		if (!part) {/*如果bdev是主分区（part==0）,调用底层驱动的打开接口，并且扫描此分区的主分区*/
			struct backing_dev_info *bdi;
			if (disk->fops->open) {/*调用gendisk对象的操作接口*/
				ret = disk->fops->open(bdev->bd_inode, file);
				if (ret)
					goto out_first;
			}
			if (!bdev->bd_openers) {/*第一次打开设备，就设备设备的容量*/
			...
			whole = bdget_disk(disk, 0);/*获取当前子分区的主分区描述符*/
			ret = -ENOMEM;
			if (!whole)
				goto out_first;
			...
			if (ret)/*扫描失败后退出*/
				goto out_first;
			/*设备此子分区bd_contains域的主分区*/
			bdev->bd_contains = whole;
			mutex_lock_nested(&whole->bd_mutex, BD_MUTEX_WHOLE);
			/*增加主分区的打开引用计数器。注：bd_part_count表示打开主分区的次数,这个主要控制扫描分区接口rescan_partitions，如果此值不为0
			  则不进行子分区的扫描*/
			whole->bd_part_count++;
			p = disk->part[part - 1];/*子分区对象地址*/
			bdev->bd_inode->i_data.backing_dev_info =
			   whole->bd_inode->i_data.backing_dev_info;
			/*如果gendisk中的flags没设置了GENHD_FL_UP标志、分区不存在、分区大小为0，则忽略当前打，即把bd_part_count引用记数忽略。*/
			...
			kobject_get(&p->kobj);
			bdev->bd_part = p;/*设备当前的块设备描述符的分区*/
			bd_set_size(bdev, (loff_t) p->nr_sects << 9);/*设置分区大小*/
			mutex_unlock(&whole->bd_mutex);
		}
	}else {/*bdev已经打开过*/	
		put_disk(disk);
		module_put(owner);
		if (bdev->bd_contains == bdev) {/*打开的是主分区*/
			if (bdev->bd_disk->fops->open) {/*调用底层驱动的打开接口*/
				ret = bdev->bd_disk->fops->open(bdev->bd_inode, file);
				if (ret)
					goto out;
			}
			if (bdev->bd_invalidated)/*扫描主分区下的子分区*/
