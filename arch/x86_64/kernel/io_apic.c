 ...
 */
/**ltl
 * 功能:根据GSI，获取IOAPIC中断向量号
 * 参数:gsi	->GSI号
 * 返回值:
 * 说明: 此中断从从16(全局变量next_irq的初始值)开始
 */
...
static int pin_2_irq(int idx, int apic, int pin)
{
	...
	switch (mp_bus_id_to_type[bus])
	{
		...
		case MP_BUS_PCI: /* PCI pin */	/* PCI设备 */
		{
			...
			irq += pin;
			irq = gsi_irq_sharing(irq); /* GSI到系统能识别的中断向量的转变 */
			break;
		}
		default:
		{
...
static void ioapic_register_intr(int irq, int vector, unsigned long trigger)
{
	...
	if ((trigger == IOAPIC_AUTO && IO_APIC_irq_trigger(irq)) ||
		...
		irq_desc[idx].chip = &ioapic_level_type; /* ioapic_level_type全局变量在io_apic.c中定义 */
	else
...
/**ltl
 * 功能: 读取IOAPIC控制器芯片编号为ioapic的引脚数(与外设的中断引脚相连的) 
 * 参数: ioapic	->IOAPIC芯片编号
 * 返回值: 当前IOAPIC芯片的输入引脚数。
 * 说明:
 */
...
int io_apic_set_pci_routing (int ioapic, int pin, int irq, int triggering, int polarity)
{
	...
	entry.dest_mode = INT_DEST_MODE;
	entry.dest.logical.logical_dest = cpu_mask_to_apicid(TARGET_CPUS);
	entry.trigger = triggering;			/* 触发方式:低电平、边缘 */
	entry.polarity = polarity;			/* 电平触发时的极性(高、低) */
	...
	/* 将外部设备使用的GSI号与IO APIC中的REDIR_TBL表建立联系 */
