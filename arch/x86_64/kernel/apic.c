...
static void setup_APIC_timer(unsigned int clocks)
{
	...
	if (vxtime.mode == VXTIME_PMTMR &&
		...
		apic_runs_main_timer++; /* 增加计数器(若此值大于1，系统不在使用0号时钟中断，参见:timer_interrupt) */
	}
	local_irq_restore(flags);
}
...
void __init setup_boot_APIC_clock (void)
{
	...
	using_apic_timer = 1; /* 标记系统使用APIC时钟 */

	local_irq_disable();

	calibration_result = calibrate_APIC_clock(); /* 校正APIC时钟 */
	/*
	 * Now set up the timer for real.
	 */
	setup_APIC_timer(calibration_result); /* 用校正的结果设置APIC时钟 */ 
 ...
 * 参数:
 * 返回值:
 * 说明: 如果系统支持APIC中断，则此函数为0xEF号时钟中断的处理函数(参见init_IRQ())；
 *       若不支持，则此函数为0号时钟中断的处理函数(见time_init()函数)
 */
...
#ifdef CONFIG_X86_IO_APIC
	/* 安装IO APIC，包括在0-16之间的中断号所使用的8259A */
	if (smp_found_config && !skip_ioapic_setup && nr_ioapics)
			setup_IO_APIC();
	else
		nr_ioapics = 0;
#endif
	setup_boot_APIC_clock(); /* 安装APIC时钟 */
	check_nmi_watchdog();
	return 0;
}
